...
def trace(operation: str, trace_id: str | None = None, **fields: Any) -> Iterator[dict[str, Any]]:
    ...
    # Merge fields and set trace_id in the merged dict to avoid duplicate kwargs
    log_event(f"{operation}_start", **{**fields, "trace_id": tid})
    ctx: dict[str, Any] = {"trace_id": tid}
    try:
        yield ctx
    finally:
        ctx["latency_ms"] = (time.perf_counter() - start) * 1000
        # Pass merged fields and ctx together — ctx already contains trace_id
